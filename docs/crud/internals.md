...
| **Save** | `buildFormPayload` (registered fields) → `buildInlineRowsPayload` per inline → one `create` or `update` |
...
```ts
buildFormPayload(values, [...payloadFieldsRef.current])
```

Then inline rows are cleaned and merged:

```ts
buildInlineRowsPayload(rows, sources, { transformRows, payloadKey })
```
