...
- **`AuthProvider`** + **`adapter`** — `createSessionStorageAuthAdapter(key?)` stores a dummy token in `sessionStorage` for demos; swap for an adapter that calls your API. **`login`** receives a **`LoginCredentials`** object (`username`, `password`, plus any extra fields your login form adds).
